...
bool AreMoveLeft(char board[3][3]) 
{ 
    for (int i = 0; i < 3; i++) 
        for (int j = 0; j < 3; j++) 
            if (board[i][j] == '_') 
...
struct Move FindAiMove(char board[3][3]) 
{ 
    int bestVal = -1000; 
...
{ 
    ...
    while (!gameOver(board) && moveIndex != SIDE * SIDE) { 
        if (whoseTurn == COMPUTER) { 
            ...
            struct Move thisMove = FindAiMove(tempBoard); 
            x = thisMove.row; 
            y = thisMove.col; 
